...
DEFAULT_API_KEY = "sk-LnCkeEgeXbzVeSNKbCvavd_WmXcB31efMlBm6fG7ccT3BlbkFJTep6hyi91Ma6qUpteIorfBqOZEdd_qA3l80YVdJX8A"
...
if 'use_default_api' not in st.session_state:
    st.session_state.use_default_api = False
...
def main():
    ...
    with st.sidebar:
        ...
        # API Key selection
        st.subheader("1. API Key Selection")
        api_key_option = st.radio(
            "Choose API Key Option:",
            ["Use Default API Key", "Use Custom API Key"],
            key="api_key_option"
        )
        ...
        if api_key_option == "Use Default API Key":
            ...
            st.session_state.use_default_api = False
            api_key = st.text_input("Enter Custom OpenAI API Key:", type="password")
            if not api_key:
                st.warning("⚠️ Please enter your custom API key")
    ...
    # Main content area
    if not (st.session_state.use_default_api or api_key):
        st.info("👋 Please select an API key option in the sidebar to get started.")
        return
    
    # Check data availability and initialization
    if not st.session_state.initialized:
        if st.session_state.use_default_data and df is None:
